...
while(1)
    ...
    for i=1:4
        if ( i==1)
         ...
         P1=Triangle5(A,B,C,tag_x(1),tag_y(1));
         PL1(i,1)=P1(1)-delt(i,1);
         PL1(i,2)=P1(2)-delt(i,2);
         P2=Triangle5(A,B,C,tag_x(2),tag_y(2));
         PL2(i,1)=P2(1)-delt(i,1);
         PL2(i,2)=P2(2)-delt(i,2);
         P3=Triangle5(A,B,C,tag_x(3),tag_y(3));
         PL3(i,1)=P3(1)-delt(i,1);
         PL3(i,2)=P3(2)-delt(i,2);  

    end        %maxrix(i,j,:)表示第i个reader 与第j个tag的p阵  dist(i,j)的距离 
        UN1(1,1)=(PL1(1,1)/dist1(1,1)+PL1(2,1)/dist1(2,1)+PL1(3,1)/dist1(3,1)+PL1(4,1)/dist1(4,1))/(1/dist1(1,1)+1/dist1(2,1)+1/dist1(3,1)+1/dist1(4,1));
        UN1(1,2)=(PL1(1,2)/dist1(1,1)+PL1(2,2)/dist1(2,1)+PL1(3,2)/dist1(3,1)+PL1(4,2)/dist1(4,1))/(1/dist1(1,1)+1/dist1(2,1)+1/dist1(3,1)+1/dist1(4,1));
        UN1(2,1)=(PL2(1,1)/dist1(1,2)+PL2(2,1)/dist1(2,2)+PL2(3,1)/dist1(3,2)+PL2(4,1)/dist1(4,2))/(1/dist1(1,2)+1/dist1(2,2)+1/dist1(3,2)+1/dist1(4,2));
       UN1(2,2)=(PL2(1,2)/dist1(1,2)+PL2(2,2)/dist1(2,2)+PL2(3,2)/dist1(3,2)+PL2(4,2)/dist1(4,2))/(1/dist1(1,2)+1/dist1(2,2)+1/dist1(3,2)+1/dist1(4,2));
       UN1(3,1)=(PL3(1,1)/dist1(1,3)+PL3(2,1)/dist1(2,3)+PL3(3,1)/dist1(3,3)+PL3(4,1)/dist1(4,3))/(1/dist1(1,3)+1/dist1(2,3)+1/dist1(3,3)+1/dist1(4,3));
       UN1(3,2)=(PL3(1,2)/dist1(1,3)+PL3(2,2)/dist1(2,3)+PL3(3,2)/dist1(3,3)+PL3(4,2)/dist1(4,3))/(1/dist1(1,3)+1/dist1(2,3)+1/dist1(3,3)+1/dist1(4,3));
       r=r+1;
end
plot(reader_x,reader_y,'r+',UN1(:,1),UN1(:,2),'go',tag_x,tag_y,'b*'),title('测试结果，红色为reader点，蓝色为tag点，绿色为测试结果，改变tag_xy(4)测试'),ylabel('y'),xlabel('x');grid on  
set(gca,'Xtick',[-1:1:11],'Ytick',[-1:1:11]);
hold on
plot([tag_x(1);UN1(1,1)],[tag_y(1);UN1(1,2)]);
plot([tag_x(2);UN1(2,1)],[tag_y(2);UN1(2,2)]);
plot([tag_x(3);UN1(3,1)],[tag_y(3);UN1(3,2)]);
for i=1:3
