...
e_average_box=zeros(1,5);
e_average_percent_box=zeros(1,5);
for t = 1:5
    A = [0,0];
    B = [5*t,5*t*sqrt(3)];
    C = [10*t,0];
...
%         e_sum=sqrt((P_calculate(i,1)-P_position(i,1))^2+(P_calculate(i,2)-P_position(i,2))^2);
        e_sum = e_sum+e;
    end
    e_average = e_sum/N;
